...
@Component({
  ...
  templateUrl: './clt-dashboard.html',
  standalone: true,
  imports: [
    AsyncPipe,
    NgForOf,
  ...
  styleUrls: ['./clt-dashboard.scss']
})
export class CltDashboardComponent implements OnInit {
  ...
  onDelete(id: number | undefined): void {
    ...
    // ✅ Remplacement de confirm() — suppression directe avec feedback
    this.candidatService.delete(id).subscribe({
      next: () => {
        this.onSearchChange();
        this.notif.success('Client supprimé avec succès.');
      },
      error: (err) => {
        console.error('Erreur lors de la suppression', err);
        this.notif.error('Erreur lors de la suppression du client.');
      }
    });
  }
  ...
  downloadFile(fullPath: string) {
    ...
    this.candidatService.downloadCv(fileName).subscribe({
      next: (blob: Blob) => {
        ...
        this.notif.success(`Fichier "${fileName}" téléchargé avec succès.`);
      },
